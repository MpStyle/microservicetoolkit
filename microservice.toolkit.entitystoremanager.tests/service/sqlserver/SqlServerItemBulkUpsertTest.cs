...
public class SqlServerItemBulkUpsertTest : MigratedDbTest
{
    ...
    public async Task Run()
    {
        ...
        var user = new MyItem
        {
            ...
            LongsValue =
                new[] {DateTimeOffset.UtcNow.ToUnixTimeMilliseconds(), DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()},
            BoolValue = true,
            BooleansValue = new[] {true, false, true},
            FloatValue = 7312.427F,
            FloatsValue = new[] {7312.427F, 7312.428F},
            IntValue = 392817,
            IntsValue = new[] {392817, 392818},
            Inserted = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds(),
            Updated = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds(),
        };
        await this.service.Run(new ItemBulkUpsertRequest<MyItem> {Items = new[] {user}});

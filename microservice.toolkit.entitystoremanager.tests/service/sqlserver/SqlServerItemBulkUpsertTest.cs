...
public class SqlServerItemBulkUpsertTest : MigratedDbTest
{
    private SqlServerItemBulkUpsert<MyItem> service;
    private SqlServerItemBulkUpsert<MyCustomItem> customService;
    ...
    public async Task Run()
    {
        ...
        var user = new MyItem
        {
            ...
            FloatsValue = [7312.427F, 7312.428F],
            IntValue = 392817,
            IntsValue = [392817, 392818],
            Inserted = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds(),
            Updated = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds(),
        };
        await this.service.Run(new ItemBulkUpsertRequest<MyItem> { Items = [user] });

        var byIdResponse =
            await new SqlServerItemById<MyItem>(this.DbConnection).Run(new ItemByIdRequest { ItemId = itemId });
    ...
    public async Task Run_Custom()
    {
        ...
        await this.customService.Run(new ItemBulkUpsertRequest<MyCustomItem> { Items = [user] });

        var byIdResponse =
            await new SqlServerItemById<MyCustomItem>(this.DbConnection).Run(new ItemByIdRequest { ItemId = itemId });
    ...
    public void SetUp()
    {
        this.service = new SqlServerItemBulkUpsert<MyItem>(this.DbConnection);
        this.customService = new SqlServerItemBulkUpsert<MyCustomItem>(this.DbConnection);
    }
